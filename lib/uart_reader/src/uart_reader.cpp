...
void recieveScooterData(HardwareSerial *miSerial, 
  ...
  if (miSerial->available() && !commState->hasPreambleFirst && !commState->hasPreambleSecond) {
  ...
  if (miSerial->available() > 0 && commState->hasPreambleFirst && commState->hasPreambleSecond && !commState->hasLength) {
    recievedData[2] = miSerial->read();
    commState->hasLength = true;
  }
  if (miSerial->available() > 0 && commState->hasLength && !commState->hasCompletedBeforeCRC) {
